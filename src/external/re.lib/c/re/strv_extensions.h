...
STRV_INTERNAL bool char_is_alphanum(char c);

/* Returns true if match regex (.[a-zA-Z0-9_])+ */
STRV_INTERNAL bool parse_email_ending(const char** cursor, const char* end);

STRV_INTERNAL void eat_email_suffix(const char** cursor, const char* end);

/* Returns true if match regex [a-zA-Z0-9_]+ */
STRV_INTERNAL bool parse_email_subdomain(const char** cursor, const char* end);
